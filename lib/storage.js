...
OpenStackStorage.prototype.getInfo = function (containerName, fileDataToReceive, callback) {
  var self = this;
  if(!fileDataToReceive || !containerName) {
    return callback(new Error("must specify remoteName and either .localFile or .stream for file metadata"));
  }

  var targetURL = url.parse(self.tokens.storageUrl + '/' + containerName + '/' + fileDataToReceive);
