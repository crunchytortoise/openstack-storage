...
OpenStackStorage.prototype.createPrefixPath = function (containerName, prefixToCreate, callback) {
  var self = this;
  if(!prefixToCreate) {
    return callback(new Error("must specify remoteName and either .localFile or .stream for file uploads"));
  }

  if(prefixToCreate.substr(-1)==="/") { prefixToCreate = prefixToCreate.substr(0, prefixToCreate.length-1) }
  if(prefixToCreate[0]==="/") { prefixToCreate = prefixToCreate.substr(1, prefixToCreate.length-1) }
  var targetURL = url.parse(self.tokens.storageUrl + '/' + containerName + '/' + prefixToCreate);
